/*****************************************************************************
* Filename:          /home/vladimir/cf_ad9467_zed/drivers/init_dma1_v1_00_a/src/init_dma1_selftest.c
* Version:           1.00.a
* Description:       Contains a diagnostic self-test function for the init_dma1 driver
...
#define INIT_DMA1_SELFTEST_BUFSIZE  64 /* Size of buffer (for transfer test) in bytes */
#define INIT_DMA1_USER_NUM_REG  32/* Number of registers in slave */

/************************** Variable Definitions ****************************/

static Xuint8 __attribute__((aligned (128))) SrcBuffer[INIT_DMA1_SELFTEST_BUFSIZE];   /* Source buffer      */
static Xuint8 __attribute__((aligned (128))) DstBuffer[INIT_DMA1_SELFTEST_BUFSIZE];   /* Destination buffer */
...
XStatus INIT_DMA1_SelfTest(void * baseaddr_p)
{
  ...
  for (write_loop_index = 0 ; write_loop_index < INIT_DMA1_USER_NUM_REG; write_loop_index++)
    INIT_DMA1_mWriteSlaveReg0 (baseaddr, write_loop_index*4, (write_loop_index+1)*READ_WRITE_MUL_FACTOR);
  for (read_loop_index = 0 ; read_loop_index < INIT_DMA1_USER_NUM_REG; read_loop_index++)
    if ( INIT_DMA1_mReadSlaveReg0 (baseaddr, read_loop_index*4) != (read_loop_index+1)*READ_WRITE_MUL_FACTOR){
  ...
  if ( (INIT_DMA1_mReadSlaveReg0 (baseaddr, 0)) != 0x0){
  ...
  * Master */
  ...
  for ( Index = 0; Index < INIT_DMA1_SELFTEST_BUFSIZE; Index++ )
  {
    SrcBuffer[Index] = Index;
    DstBuffer[Index] = 0;
  }
  xil_printf("   - start user logic master module to receive word from the source\n\r");
  INIT_DMA1_MasterRecvBytes(baseaddr, (Xuint32) SrcBuffer, INIT_DMA1_SELFTEST_BUFSIZE);
  while ( ! INIT_DMA1_mMasterDone(baseaddr) ) {}
  xil_printf("   - transfer completed\n\r");
  xil_printf("   - start user logic master module to send word to the destination\n\r");
  INIT_DMA1_MasterSendBytes(baseaddr, (Xuint32) DstBuffer, INIT_DMA1_SELFTEST_BUFSIZE);
  while ( ! INIT_DMA1_mMasterDone(baseaddr) ) {}
  xil_printf("   - transfer completed\n\r");
